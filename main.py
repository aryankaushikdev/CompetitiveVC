...
class ScoringEngine:
    ...
    def generate_insight(company: str, competitive: float, health: float, dev: Optional[float], funding: dict) -> str:
        """Generate contextual AI insight with key metrics"""
        parts = []
        ...
        if competitive >= 8:
            parts.append(
                f"Competitive Landscape: {company} demonstrates strong positioning with aggressive pricing strategy.")
        elif competitive >= 6:
            parts.append(
                f"Competitive Landscape: {company} maintains solid market positioning with clear differentiation.")
        else:
            parts.append(f"Competitive Landscape: {company} faces pricing pressure; may need GTM adjustment.")

        # Product Market Fit + Health
        if health >= 9:
            parts.append(f"Product Market Fit: Strong — {stage} with {raised} raised signals validated demand.")
        elif health >= 7:
        ...
        if health >= 8 and competitive >= 7:
            parts.append("Product Scalability: High — infrastructure and pricing support growth.")
        elif health >= 6:
        ...
        # Product IP / Technical Moat
        if dev and dev >= 8:
            parts.append("Product IP: Strong technical founder adds execution credibility and defensibility.")
        elif dev and dev >= 6:
            parts.append("Product IP: Solid technical foundation.")

        # Customer Acquisition Cost
        if funding.get("has_free_tier", False) or competitive >= 8:
            parts.append("Customer Acquisition: PLG model suggests efficient CAC.")
        else:
            parts.append("Customer Acquisition: Sales-led motion — monitor CAC payback.")
